...
export function Shop({ run, onChange }: { run: RunState; onChange: (r: RunState) => void }) {
  ...
  const apply = (r: RunState | string, sound: 'buy' | 'card' = 'buy') => {
    if (typeof r === 'string') { sfx('error'); setMsg(r); return; }
    sfx(sound);
    setMsg(null);
    onChange(r);
  };
      ...
      <div className="shop-body">
        <div className="shop-left">
          ...
          <div className="shop-row">
            ...
            <button className="btn t20" disabled={run.cash < rerollPrice(shop)} onClick={() => apply(reroll(run), 'card')}>Reroll cards ${rerollPrice(shop)}</button>
            <button className={`btn t20 ${removing ? 'danger' : ''}`} disabled={shop.removed} onClick={() => setRemoving(!removing)}>
        ...
        <div className="shop-right">
            ...
            {[...counts.entries()].map(([id, count]) => {
              ...
              return (
                <button key={id} className={`deck-row ${removing ? 'removable' : ''}`} disabled={!removing}
                  onClick={() => { apply(removeCard(run, run.cards.indexOf(id)), 'card'); setRemoving(false); }}
          ...
          <button className="btn green t40 next-day" onClick={() => { sfx('card'); onChange(tomorrow); }}>
